...
int my_read(const char* path, char* buf, size_t size, off_t offset, struct fuse_file_info* fi)
{
  ...
  while(bytes2Read) {
    int i;
    int currentBlock, offBlock;
    if ((offset / BLOCK_SIZE_BYTES) > node->numBlocks)
      return 0;
    currentBlock = node->blocks[offset/BLOCK_SIZE_BYTES];
    offBlock = offset % BLOCK_SIZE_BYTES;
...
static int my_unlink (const char *path)
{
  ...
  int fileindex;
  if((fileindex = findFileByName(&myFileSystem, (char *)path + 1)) == -1) {
    fprintf(stderr, "Error getting file index\n");
    return -1;
  }
    
  int nodeIdx = myFileSystem.directory.files[fileindex].nodeIdx;
  NodeStruct *nodeI = myFileSystem.nodes[nodeIdx];

  
  /* Marcar como libre en el bitMap */
  int i = 0;
  while(i < nodeI->numBlocks){
    myFileSystem.bitMap[nodeI->blocks[i]] = 0;
    i++;
  }
  
  /* Eliminar del directorio raiz:
   * - Marcar entrada como libre
   * - Decrementar número de entradas
   */
  myFileSystem.directory.files[fileindex].freeFile = true;
  myFileSystem.directory.numFiles--;

  /* Actualizar superbloque:
   * - diskSizeInBlocks
   */
  myFileSystem.superBlock.numOfFreeBlocks += nodeI->numBlocks;

  /* Liberar nodo-i : freeNode=True*/
  nodeI->freeNode=true;
  
  /* Aumentar numFreeNodes */
  myFileSystem.numFreeNodes++;

  updateBitmap(&myFileSystem);
  updateDirectory(&myFileSystem);
  updateSuperBlock(&myFileSystem);
  updateNode(&myFileSystem, nodeIdx, nodeI);
  return 0;
}
...
struct fuse_operations myFS_operations = {
    ...
    .read       = my_read,                                      // Read the contents of a file
};
